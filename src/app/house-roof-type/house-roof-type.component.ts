...
import {HouseRoofType} from "./house-roof-type.model";
...
export class HouseRoofTypeComponent implements OnInit {
  ...
  @Input() roofTypeStep;
  shapes: HouseRoofType[];

  selectedShape;
  public  twoSideOccupy: boolean = false;
  ...
  public getIconClass(shape) {
    // console.log('Called', shape.key, this.selectedShape)
    if (this.selectedShape && this.selectedShape.key === shape.key) {
      return shape.seq + ' selected';
    }
    return shape.seq;
  }
  ...
  public selectShape(shape): void {
    this.selectedShape = shape;
  }

  public submitForm(): void {
    this.onSubmit.emit({
      value: this.selectedShape , twoSideOccupy: this.twoSideOccupy
    });
  }
